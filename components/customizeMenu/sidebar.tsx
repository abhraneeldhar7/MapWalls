...
import { ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, CheckIcon, CopyIcon, MinusIcon, PlusIcon, Tick02Icon } from "@/components/ui/icons";
...
function LocationRow() {
  ...
  const value = typed ?? currentText;
  const dirty = typed !== null && typed !== currentText;
  ...
  const onSubmit = (e: FormEvent) => {
    ...
    const [lon, lat] = typed.split(",").map((s) => parseFloat(s.trim()));
    if (isNaN(lon) || isNaN(lat)) return;
    const clon = Math.min(180, Math.max(-180, lon));
    const clat = Math.min(90, Math.max(-90, lat));
    setViewState({ ...viewState, longitude: clon, latitude: clat });
    setTyped(null);
    setCopied(false);
  };
    ...
    <form className="flex flex-col gap-1.5" onSubmit={onSubmit}>
      ...
      <div className="flex items-center justify-between gap-1">
        <Input
          ...
          className="flex-1 h-8 sm:h-8"
        />
        <Button type="submit" variant="secondary" size="icon-sm">
          {dirty ? <CheckIcon /> : copied ? <Tick02Icon /> : <CopyIcon />}
        </Button>
      </div>
    </form>
  );
...
export default function CustomizeSidebar() {
  ...
  const { editorState, setEditorState, setFocusedElement, elements, updateElement, addElement, canvas, mapElement } = useEditor();
  ...
  const editorGet = (path: string) => {
    if (path === "canvas.color") return canvas.color;
    if (path === "canvas.aspectRatio") return getAspectValue(canvas);
    if (path === "map.aspectRatio") return mapElement ? getAspectValue(mapElement) : undefined;
    return undefined;
  };
  const editorSet = (paths: string[], value: unknown) => {
    const v = String(value);
    if (paths.includes("canvas.color")) updateElement("canvas", { color: v });
    if (paths.includes("canvas.aspectRatio")) updateElement("canvas", applyAspectRatio(canvas, v));
    if (paths.includes("map.aspectRatio") && mapElement) updateElement(mapElement.id, applyAspectRatio(mapElement, v));
  };
